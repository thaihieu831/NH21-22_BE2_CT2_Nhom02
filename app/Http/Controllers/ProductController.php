...
class ProductController extends Controller
{
    ...
    public function category_product($type_id)
    {
        $category_product = Product::leftJoin('protypes', 'products.type_id', '=', 'protypes.type_id')->where('products.type_id', '=', $type_id)->paginate(3);
        if ($key = request()->key) {
            $category_product = Product::leftJoin('protypes', 'products.type_id', '=', 'protypes.type_id')->orderBy('product_id', 'DESC')
                ->where('product_name', 'like', '%' . $key . '%', 'or', 'price', 'like', '%' . $key . '%')->paginate(3);
        }
        return view('store', compact('category_product'));
    }
}
